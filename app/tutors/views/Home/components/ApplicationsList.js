...
const ApplicationsList = () =>
  <>
    ...
    <form>
      <input className="form-control form-control-lg" type="text" placeholder="Поиск"/>
    </form>

    <div className="list-group list-group-flush">
      {map(applications, (application, index) => (
        <Link to={routes.tutors.application.replace(':id', index)} key={index} className="list-group-item list-group-item-action">
          <div className="d-flex w-100 justify-content-between">
            <h5 className="mb-1">{application.displayName} <span className="badge badge-success">{application.status}</span></h5>
